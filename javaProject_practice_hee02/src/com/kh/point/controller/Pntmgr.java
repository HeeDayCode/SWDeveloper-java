...
public class Pntmgr {
	private Grade[] s = new Silver[10];
	private Grade[] g = new Gold[10];	
	private Grade[] v = new Vip[10];
	private Grade[] vv = new Vvip[10];
	...
	public void printData() {
		...
		for (int i = 0; i < vv_index; i++) {
			System.out.println(this.vv[i].getName() + "\t\t" + this.vv[i].getGrade() + "\t\t" + this.vv[i].getPoint()
					+ "\t\t" + this.vv[i].getInterest());
		}
	}
}